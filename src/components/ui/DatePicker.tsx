...
export default function DatePicker({ value, onChange, includeTime = false, label }: Props) {
  ...
  useEffect(() => {
    if (open && timeRef.current) {
      const active = timeRef.current.querySelector('[data-active="true"]') as HTMLElement
  ...
  const handleDayClick = (day: number) => {
    ...
    const newDate = new Date(viewYear, viewMonth, day, current.getHours() || 8, current.getMinutes() || 0)
    onChange(includeTime ? toLocalISOString(newDate) : toLocalISOString(newDate).slice(0, 10))
  }
  ...
  const picker = open ? createPortal(
    ...
    >
      <div style={{ padding: '16px', width: '260px' }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'14px' }}>
          <button type="button" onMouseDown={e => e.stopPropagation()}
            ...
            style={{ background:'none', border:'none', cursor:'pointer', color: c.text2, fontSize:'18px', lineHeight:1, padding:'2px 8px' }}
          >‹</button>
          <span style={{ fontSize:'13px', fontWeight:600, color: c.text1 }}>{MONTHS[viewMonth]} {viewYear}</span>
          <button type="button" onMouseDown={e => e.stopPropagation()}
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(7,1fr)', gap:'2px', marginBottom:'6px' }}>
          {DAYS.map(d => (
            <div key={d} style={{ textAlign:'center', fontSize:'11px', color: c.text2, padding:'4px 0', fontWeight:500 }}>{d}</div>
          ))}
        </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(7,1fr)', gap:'2px' }}>
          ...
          {Array.from({length: daysInMonth}).map((_,i) => {
            ...
            return (
              <button key={day} type="button" onMouseDown={e => e.stopPropagation()}
      ...
      {includeTime && (
        <div style={{ width:'90px', borderLeft:`1px solid ${c.border}`, display:'flex', flexDirection:'column', height:'320px' }}>
          <div style={{ padding:'14px 12px 8px', fontSize:'11px', color: c.text2, fontWeight:500, textAlign:'center', flexShrink:0 }}>ORA</div>
          <div ref={timeRef} style={{ overflowY:'auto', flex:1, padding:'4px 6px 8px' }}>
            {timeSlots.map(slot => {
              const isActive = slot === selectedTime
              return (
                <button key={slot} type="button" data-active={isActive}
                  onMouseDown={e => e.stopPropagation()}
  ...
  return (
    <div ref={triggerRef} style={{ position: 'relative' }}>
      {label && (
        <label style={{ display:'block', fontSize:'13px', color: c.text2, marginBottom:'6px' }}>{label}</label>
      )}
      <div
