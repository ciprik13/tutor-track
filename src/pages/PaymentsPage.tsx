...
export default function PaymentsPage() {
  ...
  students.forEach((student) => {
    ...
    months.forEach((month) => {
      ...
      const paidCount = monthLessons.filter(
        (l) => l.paymentStatus === "paid",
      ).length;
      const unpaidCount = monthLessons.filter(
        (l) => l.paymentStatus === "unpaid",
      ).length;

      const status: "paid" | "unpaid" | "partial" =
        paidCount === monthLessons.length
          ? "paid"
          : unpaidCount === monthLessons.length
            ? "unpaid"
            : "partial";
  ...
  const handleMarkAllPaid = async (summary: MonthSummary) => {
    const monthLessons = allLessons.filter(
      (l) =>
  ...
  const handleMarkAllUnpaid = async (summary: MonthSummary) => {
    ...
    await Promise.all(
      monthLessons.map((l) =>
        updateLesson.mutateAsync({ ...l, paymentStatus: "unpaid" }),
      ),
    );
    queryClient.invalidateQueries({ queryKey: ["lessons"] });
  };
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-4 sm:mb-6 gap-3 sm:gap-0">
          ...
          {totalUnpaid > 0 && (
            <div className="text-left sm:text-right">
              ...
              <p className="text-xl sm:text-2xl font-bold text-[#c07a20] mt-1">
                {Math.round(totalUnpaid)} {profile.currency}
              </p>
            </div>
          )}
        </div>
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-3">
                        <p
                          ...
                          onClick={() =>
                            navigate(`/students/${summary.studentId}`)
                          }
                        >
                          {summary.studentName}
                        </p>
                      ...
                      <p className="text-(--text-3) text-xs mt-0.5">
                        ...
                        {summary.paidCount > 0 &&
                          ` · ${summary.paidCount} achitate`}
                        {summary.unpaidCount > 0 &&
                          ` · ${summary.unpaidCount} neachitate`}
                      </p>
                    </div>

                    <div className="text-right mr-4">
                      <p className="text-(--text-1) font-bold text-lg">
                        {summary.total} {profile.currency}
                      </p>
                      {summary.status === "partial" && (
                        <p className="text-xs text-[#c07a20] mt-0.5">
                          {Math.round(
                            (summary.total * summary.unpaidCount) /
                              summary.lessonCount,
                          )}{" "}
                          neachitat
                        </p>
