...
export default function StudentDetailPage() {
  ...
  const [activeTab, setActiveTab] = useState<"lessons" | "payments" | "report">(
    "lessons",
  );
  ...
  const unpaidLessons = lessons.filter(
    (l) => l.paymentStatus === "unpaid" && l.status === "done",
  );
  const unpaidTotal = unpaidLessons.reduce(
    (sum, l) => sum + l.pricePerSession,
    0,
  );
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 mb-4 sm:mb-6">
          ...
          <div className="flex-1">
            <div className="flex flex-wrap items-center gap-2 sm:gap-3">
              ...
              {student.priority && (
                <span className="text-xs bg-[#c07a20]/10 text-[#c07a20] px-2 py-0.5 rounded-full">
                  Prioritar
                </span>
              )}
              <span
        ...
        {unpaidLessons.length > 0 && (
          <div className="bg-[#c07a20]/10 border border-amber-400/20 rounded-xl p-4 mb-4 sm:mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div>
              <p className="text-[#c07a20] text-xs font-medium uppercase tracking-wider">
                Lecții neachitate
              </p>
            ...
            {lessons.length === 0 ? (
              ...
              <div className="grid gap-3">
                {lessons.map((lesson) => (
                  <div
                    key={lesson.id}
                    className="bg-(--bg-card) border border-(--border) rounded-xl p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 hover:border-(--border) transition-colors"
                  >
              ...
              <div className="grid gap-3">
                {payments.map((payment) => (
                  ...
                  >
                    ...
                    <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                      <p className="text-(--text-1) font-bold">
        ...
        {activeTab === "report" && (
          <div className="text-(--text-2) text-sm py-8 text-center">
            <p>
              Mergi la pagina{" "}
              <span
                className="text-[#52ab98] cursor-pointer"
                onClick={() => navigate("/reports")}
              >
                Rapoarte
              </span>{" "}
              pentru a genera raportul lunar
            </p>
          </div>
        )}
      </div>
