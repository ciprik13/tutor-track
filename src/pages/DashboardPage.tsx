...
export default function DashboardPage() {
  ...
  const lessonsThisMonth = allLessons.filter(
    (l) => l.date.startsWith(currentMonth) && l.status === "done",
  );

  const lessonsThisWeek = allLessons.filter(
    (l) => l.date.slice(0, 10) >= currentWeekStart && l.status === "done",
  );
    ...
    <div className="min-h-full p-6">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
          ...
          >
            <p className="text-(--text-2) text-xs uppercase tracking-wider mb-3">
              Studenți activi
            </p>
            <p className="text-(--text-1) font-bold text-3xl">
              {activeStudents.length}
            </p>
            <p className="text-(--text-3) text-xs mt-2">
              din {students.length} total
            </p>
          </div>
          ...
          >
            ...
            <p className="text-(--text-3) text-xs mt-2">
              {lessonsThisMonth.reduce((s, l) => s + l.pricePerSession, 0)}{" "}
          ...
          >
            ...
            <p
              className={`font-bold text-3xl ${unpaidTotal > 0 ? "text-[#c07a20]" : "text-[#52ab98]"}`}
            >
              {unpaidTotal}
            </p>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-(--text-2) text-xs mt-0.5">
                          {formatDate(lesson.date)} · {lesson.durationMinutes}{" "}
                          min
                        </p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-(--text-1) text-sm font-bold">
                        {lesson.pricePerSession} {profile.currency}
                      </p>
                      <p
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-(--text-2) text-xs mt-0.5">
                            {student.subject} · {studentLessons.length} lecții
                            luna aceasta
                          </p>
                        </div>
                      </div>
      ...
      {lessonModal && (
        <LessonModal lesson={null} onClose={() => setLessonModal(false)} />
      )}
    </div>
  );
}
