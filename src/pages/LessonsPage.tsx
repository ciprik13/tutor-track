...
export default function LessonsPage() {
  ...
  const googleConnected = useSelector((state: RootState) => state.profile.googleCalendarConnected)
  ...
  const handleEdit = (lesson: Lesson) => { setEditLesson(lesson); setModalOpen(true) }
  ...
  const handleToggle = (lesson: Lesson) => {
    togglePayment.mutate({ id: lesson.id!, paymentStatus: lesson.paymentStatus === 'paid' ? 'unpaid' : 'paid' })
  }

  const getStudentName = (studentId: number) => students.find(s => s.id === studentId)?.name ?? '—'
  const formatDate = (iso: string) => new Date(iso).toLocaleDateString('ro-RO', { day: 'numeric', month: 'short', year: 'numeric' })
    ...
    <div className="min-h-full p-4 md:p-6">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-4 md:mb-6 gap-3">
          <div>
            <h1 className="text-xl md:text-2xl font-bold text-(--text-1) tracking-tight">Lecții</h1>
            <p className="text-(--text-2) text-sm mt-0.5">{lessons.length} lecții</p>
          </div>
        ...
        <div className="flex flex-wrap gap-2 md:gap-3 mb-4 md:mb-6">
          <select
            ...
            className="bg-(--bg-card) border border-(--border) rounded-lg px-3 py-2 text-sm text-(--text-1) focus:outline-none focus:border-[#2b6777] transition-colors min-w-0 flex-1 md:flex-none"
          >
        ...
        {isLoading ? (
          ...
          <div className="grid gap-2 md:gap-3">
            {lessons.map(lesson => (
              <div key={lesson.id} className="bg-(--bg-card) border border-(--border) rounded-xl p-3 md:p-4">
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-(--text-2) text-xs mt-0.5 truncate">
                        {formatDate(lesson.date)} · {lesson.durationMinutes} min · {lesson.pricePerSession} MDL
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-1 md:gap-2 flex-shrink-0">
                    ...
                    <span className={`hidden md:inline text-xs px-2.5 py-1 rounded-full ${
                      lesson.status === 'done' ? 'bg-(--bg-input) text-(--text-3)' : 'bg-red-400/10 text-red-400'
                    }`}>
