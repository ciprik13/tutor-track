...
export default function StatisticsPage() {
  ...
  const yearLessons = allLessons.filter(l =>
    l.date.startsWith(String(currentYear)) && l.status === 'done'
  )
  ...
  const monthlyData = MONTHS_SHORT.map((month, i) => {
    ...
    const achitat = monthLessons.filter(l => l.paymentStatus === 'paid').reduce((s, l) => s + l.pricePerSession, 0)
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    ...
    return (
      <div style={{ background: tooltipBg, border: `1px solid ${tooltipBorder}`, borderRadius: '8px', padding: '10px 14px' }}>
        <p style={{ fontSize: '12px', color: textColor, marginBottom: '6px' }}>{label}</p>
        {payload.map((p: any) => (
    ...
    <div style={{ minHeight: '100%', padding: '24px', background: isDark ? '#0a0a0f' : '#f5f4f0' }}>
      <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: '28px' }}>
          <h1 style={{ fontSize: '24px', fontWeight: 700, color: isDark ? '#f0f0f2' : '#18181c', letterSpacing: '-0.4px' }}>
            Statistici
          </h1>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px', marginBottom: '24px' }}>
          {[
            ...
            <div key={card.label} style={{ ...cardStyle }}>
              <p style={{ fontSize: '11px', color: textColor, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '10px' }}>{card.label}</p>
              <p style={{ fontSize: '22px', fontWeight: 700, color: isDark ? '#f0f0f2' : '#18181c', lineHeight: 1 }}>{card.value}</p>
              <p style={{ fontSize: '11px', color: textColor, marginTop: '6px' }}>{card.sub}</p>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
          <div style={cardStyle}>
            <p style={{ fontSize: '13px', fontWeight: 600, color: isDark ? '#f0f0f2' : '#18181c', marginBottom: '20px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
          <div style={{ ...cardStyle, display: 'flex', alignItems: 'center', gap: '24px' }}>
            <div>
              ...
              <ResponsiveContainer width={160} height={160}>
                <PieChart>
                  <Pie data={studentData} cx="50%" cy="50%" innerRadius={45} outerRadius={70} dataKey="value" startAngle={90} endAngle={-270}>
            ...
            <div style={{ flex: 1 }}>
              <p style={{ fontSize: '32px', fontWeight: 700, color: isDark ? '#f0f0f2' : '#18181c', lineHeight: 1 }}>
                {students.length}
              </p>
              <p style={{ fontSize: '12px', color: textColor, marginTop: '4px', marginBottom: '16px' }}>studenți total</p>
              {studentData.map(s => (
                <div key={s.name} style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                  ...
                  <span style={{ fontSize: '12px', fontWeight: 600, color: isDark ? '#f0f0f2' : '#18181c', marginLeft: 'auto' }}>{s.value}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ ...cardStyle, display: 'flex', alignItems: 'center', gap: '24px' }}>
            <div>
              ...
              <ResponsiveContainer width={160} height={160}>
                <PieChart>
                  <Pie data={durationData} cx="50%" cy="50%" innerRadius={45} outerRadius={70} dataKey="value" startAngle={90} endAngle={-270}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <p style={{ fontSize: '12px', color: textColor, marginTop: '4px', marginBottom: '16px' }}>lecții în {currentYear}</p>
              {durationData.map(d => (
                <div key={d.name} style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                  <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: d.color, flexShrink: 0 }} />
                  <span style={{ fontSize: '12px', color: textColor }}>{d.name}</span>
                  <span style={{ fontSize: '12px', fontWeight: 600, color: isDark ? '#f0f0f2' : '#18181c', marginLeft: 'auto' }}>{d.value}</span>
                </div>
              ))}
            </div>
          </div>
